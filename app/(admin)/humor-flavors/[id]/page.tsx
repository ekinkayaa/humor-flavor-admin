...
export default function FlavorDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Load captions generated by this flavor
    const { data: captionData } = await supabase
      .from("captions")
      .select("id, content, like_count, created_datetime_utc, profiles!profile_id(email, first_name, last_name)")
      .eq("humor_flavor_id", flavorId)
      .order("created_datetime_utc", { ascending: false })
      .limit(200);
    setCaptions((captionData ?? []) as unknown as GeneratedCaption[]);
    ...
    <div style={{ padding: "36px 40px" }}>
      ...
      <div style={{ marginTop: 32 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <div>
            <h2 style={{ fontSize: 17, fontWeight: 800, color: "var(--text)", margin: "0 0 3px" }}>Generated Captions</h2>
            <p style={{ fontSize: 13, color: "var(--text3)", margin: 0 }}>
              {captionsLoading ? "Loading…" : `${captions.length} caption${captions.length !== 1 ? "s" : ""} produced by this flavor`}
            </p>
          </div>
          {captions.length > 10 && (
            <button
            ...
            >
              {showAllCaptions ? "Show Less" : `Show All ${captions.length}`}
            </button>
          )}
        </div>

        {captionsLoading ? (
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "32px 24px", textAlign: "center", color: "var(--text3)", fontSize: 14 }}>
            Loading captions…
          </div>
        ) : captions.length === 0 ? (
          <div style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: 12, padding: "40px 24px", textAlign: "center", color: "var(--text3)", fontSize: 14 }}>
            <div style={{ fontSize: 22, marginBottom: 10 }}>◎</div>
            No captions have been generated with this flavor yet.
            <br />
            <span style={{ fontSize: 12 }}>Use the Test Flavor page to generate some.</span>
          </div>
        ) : (
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: "var(--surface2)", borderBottom: "1px solid var(--border)" }}>
                  {["Caption", "Author", "❤️", "Date"].map((h) => (
                    <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "var(--text3)", letterSpacing: "0.06em", textTransform: "uppercase" as const, whiteSpace: "nowrap" as const }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {(showAllCaptions ? captions : captions.slice(0, 10)).map((c, i) => {
                  const profile = c.profiles;
                  const author = profile
                    ? [profile.first_name, profile.last_name].filter(Boolean).join(" ") || profile.email
                    : "—";
                  return (
                    <tr key={c.id} style={{ borderBottom: i < (showAllCaptions ? captions : captions.slice(0, 10)).length - 1 ? "1px solid var(--border2)" : "none" }}>
                      <td style={{ padding: "10px 16px", fontSize: 13, color: "var(--text)", maxWidth: 420, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>
                        {c.content}
                      </td>
                      <td style={{ padding: "10px 16px", fontSize: 12, color: "var(--text3)", whiteSpace: "nowrap" as const, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis" }}>
                        {author}
                      </td>
                      <td style={{ padding: "10px 16px", fontSize: 13, fontWeight: 700, color: "var(--text2)", whiteSpace: "nowrap" as const }}>
                        {c.like_count ?? 0}
                      </td>
                      <td style={{ padding: "10px 16px", fontSize: 12, color: "var(--text3)", whiteSpace: "nowrap" as const }}>
            ...
            {!showAllCaptions && captions.length > 10 && (
              <div style={{ padding: "12px 16px", borderTop: "1px solid var(--border)", textAlign: "center" }}>
                <button onClick={() => setShowAllCaptions(true)} style={S.btn("secondary", true)}>
                  Show all {captions.length} captions
                </button>
              </div>
            )}
          </div>
